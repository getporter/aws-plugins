...
// Install the aws plugin to PORTER_HOME
func Install() {
...
// TestIntegration runs integration tests, requires AWS_* environment variables set
...
func XBuildAll() {
	rebuild, err := target.Dir(filepath.Join(binDir, "dev/aws-linux-amd64"), srcDirs...)
	if err != nil {
		mgx.Must(fmt.Errorf("error inspecting source dirs %s: %w", srcDirs, err))
	}
	if rebuild {
